<template>
	<div class="section_wrapper mt-4">
		<div class="section_top mb-4">
			<h2 class="pb_8 title16">Recent profile views</h2>
		</div>
			...
			<Link :href="'/u/' + log.profile.username" class="updates p_8" v-for="log in logs">
				...
				<div class="update_right">
					...
					<div class="d-flex align-items-center justify-content-between">
						<p class="text_xsm place_name">{{ log.profile.city.name }}</p>
						<p class="text_bodyColor50 text_xsm" :title="log.created_at_title">Viewed {{ log.created_at_display }}</p>
					</div>
				</div>
			</Link>
