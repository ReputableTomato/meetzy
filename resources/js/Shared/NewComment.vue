<template>
    <div class="quick_post_top pt-4">
        <a href="#" class="profile_img profile_img_48">
            <img :src="post.user.profile_picture.url" :alt="post.user.first_name + '\'s profile picture'">
        </a>
        <div class="quick_post_text position-relative">
            <div class="input-group ps-4">
                <Editor ref="editor" :id="'new-comment-' + this.post._id" @input="onEditorInput" />

                <span class="input-group-text" :class="$store.state.isNightMode ? 'bg-dark' : 'bg-light'">
                    <Recorder @audio-recorded="onAudioRecorded" />
                </span>
                <span class="input-group-text" :class="$store.state.isNightMode ? 'bg-dark' : 'bg-light'">
                    <button type="button" class="site_icon" @click="addNewComment">
    ...
    <div class="upload_area" v-if="recordedAudios.length > 0">
